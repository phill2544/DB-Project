<?php
    ...
    if(isset($_SESSION["Uid"])){
        ...
        $sql = "SELECT activity.* ,user.* , category.* FROM activity 
                ...
                WHERE activity.U_id = '$Uid' AND activity.A_state NOT IN ('กำลังดำเนินการ') ORDER BY A_id DESC";
...
<body>
    ...
    <!-- Content -->
        <!-- ช่องค้นหาจะใช้สถานะกิจกรรมในการแยกหมวด -->
        <?php if(isset($_SESSION["username"])){  ?>
            <br>
                                ...
                                <?php if(isset($_SESSION["Uid"])){ while($row = mysqli_fetch_assoc($result)){ ?>
                                    <div class="col">
                                        <div class="card text-black " style="width: 20rem; ">
                                            <!-- ส่วนหัวตรงหน้า history จะเเบ่งสีเป็น 3 สีตามสถานะ-->
                                            <?php if($row["A_state"]=="ยกเลิก"){?>
                                                ...
                                                <?php } else if( $row["A_state"]=="ครบกำหนด"){ ?>
                                                    <!-- สีเหลือง แจ้งเตือนครบกำหนดเเล้ว -->
